...
def handlePreProcessing(forcePreProcessing, method, typeOfDrift, conceptSplitSize, dataset, startDate):

    if method == 1:
        # Enter folder containing data
        os.chdir("../data/" + dataset + "/numerical")
    ...
    if forcePreProcessing:
        ...
        for stockName in stocks:
            ...
            if typeOfDrift == 0:
                ...
                sentiment = pandas.read_csv("../sentiment-scores/" + stockName, header=0)
...
def predict(forcePreProcessing, method, dataset, conceptSplitSize=5, typeOfDrift=0):
    """
    ...
    if dataset == "stocknet":
        ...
        printMe("enter a valid dataset value")
        exit()

    handlePreProcessing(forcePreProcessing, 1, typeOfDrift, conceptSplitSize, dataset, startDate)
    handlePreProcessing(forcePreProcessing, 2, typeOfDrift, conceptSplitSize, dataset, startDate)

    if os.path.exists("../../../../models/advAlstmTemp-" + dataset) and os.path.isdir(
            "../../../../models/advAlstmTemp-" + dataset):
        shutil.rmtree("../../../../models/advAlstmTemp-" + dataset)

    os.mkdir("../../../../models/advAlstmTemp-" + dataset)
    copy_tree("../../../../models/advAlstmTemp-" + dataset + " - Copy", "../../../../models/advAlstmTemp-" + dataset)

    # Create model
    pure_LSTM = AWLSTM(
        data_path=os.listdir('.')[0] + '/' + os.listdir('.')[0],
        model_path="../../../../models/advAlstm-" + dataset + "/model",
        ...
        tra_date=startDate, val_date=startDate, tes_date=startDate, att=1,
    ...
    for i in range(len(listOfConcepts)):
        ...
        if i + 1 != len(listOfConcepts) and ((method == 1 and listOfConcepts[i][:-1] == listOfConcepts[i + 1][:-1]) or (
            ...
            if first:
                pure_LSTM.set_model_path("../../../../models/advAlstmTemp-" + dataset + "/model")
                first = False
        ...
        if isMethod3 and i + 1 != len(listOfConcepts) and listOfConcepts[i].split(' ', 1)[0] != \
            ...
            pure_LSTM.set_model_path("../../../../models/advAlstm-" + dataset + "/model")
